...
*   **시스템 로직**: 
    ...
    3.  첫 메시지를 '조치 계획'으로 분류하여 히스토리에 등록.
*   **UI 변화**: 고객에게도 이때부터 우측 소통 영역이 노출됩니다.

### **단계 4: 업무 수행 및 소통 (Execution)**
*   **연기 요청 프로세스**:
    *   **ADMIN/STAFF**: 업무 지연이 예상될 경우 상세 페이지 상단에서 '연기 요청'을 수행합니다. (티켓당 **딱 1회만 가능**하며, 요청 시 버튼이 즉시 사라집니다.)
    *   **CUSTOMER**: 상세 페이지 상단 알림 영역에서 연기 요청을 확인하고 '승인' 또는 '반려'를 결정합니다.
        *   **승인 시**: 제안된 날짜가 `delayed_end_date`로 저장되며, 이후 시스템은 이 날짜를 기준으로 지연 여부를 판단합니다.
        *   **반려 시**: 반려 사유를 입력해야 하며, 해당 사유는 운영진 상세 페이지 좌측 정보 영역 하단에 노출됩니다.
*   **자동 지연 모니터링**: 
...
*   **특징**: 
    ...
    *   디자인: 
        *   별도의 타이틀이나 아이콘 없이 작성자/일시 헤더와 본문으로 구성.
        *   헤더는 회색, 내용은 검정색(`text-lg`) 폰트 적용.
        *   여백은 `space-y-4` (16px) 수준으로 컴팩트하게 유지.

### **4.2. 댓글 및 진행 히스토리 (General Comments)**
*   **정의**: 조치 계획 이후 발생하는 모든 상호 커뮤니케이션 내역.
*   **노출 위치**: 상세 페이지 **우측 채팅 영역** (스크롤 가능한 리스트).
*   **특징**:
    *   메신저 형태의 UI 적용 (본인 우측 정렬, 타인 좌측 정렬).
    *   타이틀 바를 제거하여 스크롤 영역을 최대화함.
    *   본문 폰트 크기 `text-base` (16px) 적용.
    *   자동 하단 스크롤 기능 탑재.
