...
| **`REQUESTED`** | **요청** | 연기 요청 등 운영진의 요청이 고객의 승인을 기다리는 상태 | `#242F67` (Navy) |
...
*   **`ACCEPTED` → `IN_PROGRESS`**: 상세 페이지 우측 영역에서 운영진이 **[전송 및 업무 시작]** 버튼을 눌러 메시지를 발송하면 변경됩니다. 
    *   이때 생성된 첫 메시지는 **`조치 계획`**으로 분류되어 관리됩니다.
    *   **확정 종료일자 결정**: 
        1. 운영진이 날짜를 변경했다면 변경된 날짜를 저장.
        2. 날짜 변경 섹션이 숨겨졌거나(오늘/과거인 경우) 선택하지 않았다면 **최초 희망 종료일을 자동으로 확정 종료일자로 저장**.
    *   최종 확정된 종료일자와 인력 배치가 저장됩니다.
...
*   **잔여일자(D-Day) 표시**: 
    ...
    *   **미래 (D-1까지)**: 회색 (`#9CA3AF`)으로 표시합니다.
    *   **오늘/지연 (D-Day부터)**: 빨간색 (`#E53E3E`)으로 표시합니다.
